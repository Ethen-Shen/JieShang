...
router.post('/', async (req, res) => {
    try {
        ...
        const sql = `INSERT INTO consultations (name, phone, region, address, house_type, area, budget, preferred_time, remark, source) 
        ...
        // 添加初始跟进记录
        await db.query(
            'INSERT INTO consultation_logs (consultation_id, action, new_status, content) VALUES ($1, $2, $3, $4)',
            [insertId, 'create', 'pending', '客户提交预约申请']
        );
...
router.post('/:id/assign', async (req, res) => {
    try {
        ...
        const [old] = await db.query('SELECT status FROM consultations WHERE id = $1', [req.params.id]);
        if (old.length === 0) return res.status(404).json({ error: '预约不存在' });
        
        await db.query(
            'UPDATE consultations SET employee_id = $1, status = $2 WHERE id = $3',
            [empId, 'assigned', req.params.id]
        );
        
        // 添加跟进记录
        await db.query(
            'INSERT INTO consultation_logs (consultation_id, employee_id, action, old_status, new_status, content) VALUES ($1, $2, $3, $4, $5, $6)',
...
router.post('/:id/status', async (req, res) => {
    try {
        ...
        await db.query('UPDATE consultations SET status = $1 WHERE id = $2', [status, req.params.id]);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        await db.query(
            'UPDATE consultations SET name=$1, phone=$2, region=$3, address=$4, house_type=$5, area=$6, budget=$7, preferred_time=$8, remark=$9 WHERE id=$10',
...
router.delete('/:id', async (req, res) => {
    try {
        await db.query('DELETE FROM consultations WHERE id = $1', [req.params.id]);
        res.json({ success: true });
    } catch (error) { res.status(500).json({ error: error.message }); }
});
